...
12. `tabule_sledovac.py` — sledovač úradných tabúľ stavebných úradov (14 MČ, 5 systémov: trimel, AlejTech, WordPress, Galileo, uradne.sk + Rusovce). Archív `tabule_zaznamy.json` (vyhlášky z tabúľ po 15 dňoch miznú — archív je jediná história), z PDF druh rozhodnutia, stavba, stavebník (len právnická osoba), parcely → kataster → bod. Záznam sediaci na zámer (IČO, parcela alebo 2 významné slová, pri remíze nič) → fáza doložená vyhláškou; bez zámeru → vrstva `tabule-stavby.geojson`. Spúšťať týždenne. Ružinov a Jarovce zatiaľ nie (weby z našej siete neodpovedali).