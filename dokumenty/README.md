...
6. `nahlady3.py` — náhľady grafických podkladov zo všetkých spisov, strany otočené podľa smeru písma, každý dokument v samostatnom procese s limitom 2 min (jeden vektorový výkres vie MuPDF zaseknúť na desiatky minút). `nahlady3.py 8` je skúška bez zápisu. **Nahradil ho `nahlady4.py`** (nižšie), ostáva ako záloha.
6a. `eia_dokumenty.py` — celý zoznam dokumentov ku každému zámeru z EIA API (`/api/eia_projects/<seoId>`), vrátane ďalších konaní k projektu. Výstup `mapa-zamerov/dokumenty.json` (názov, číslo dokumentu, typ, veľkosť, dátum, krok konania, sekcia). Mapa ho sťahuje až pri otvorení záložky Dokumenty, do geojsonu by pridal ~1,5 MB. `eia_dokumenty.py 3` je skúška bez zápisu.
6b. `nahlady4.py` — náhľady nad celým spisom: `--migruj` (premenuje náhľady z `nahlady3.py` na `d<číslo dokumentu>.jpg` a priradí ich k dokumentom), `--pdf` (dorobí náhľady z grafických PDF do 6 MB, najviac 6 na zámer), `--zip` (stiahne archívy do 80 MB, uloží zoznam súborov v nich do `zc` a vyrenderuje náhľady najviac 4 grafických súborov z archívu), `--zapis` (prepočíta `plany`, `nahlad`, `nahlady`, `dok` v geojsone). Rozpočty `--rozpocet <GB>` a `--limit <ks>`; enviroportál nepodporuje HTTP Range, archív sa musí stiahnuť celý. Sťahuje sa po kusoch na disk (`_docasne/`), render beží v detskom procese nad súborom — prvá verzia držala archív v pamäti a kópiu posielala do detského procesu, pri 80 MB archívoch to zhodilo beh. Typ z registra je len indícia, každé tretie „PDF“ je v skutočnosti ZIP, rozhoduje obsah súboru.
6c. `stromy_pasport.py` — stromy v správe mesta z pasportu drevín (Geoportál, `zp/pasport_STROMY_all`, 281 887 stromov): len poloha, výška a druh, delta kódované do `mapa-zamerov/stromy.json` (~3 MB, mapa ho sťahuje až od zoomu 14). Pasport nesie aj mená dendrológov, e-maily a donorov, tie sa nikdy nesťahujú. `stromy_mesta.py` — 4 994 stromov vysadených mestom od 2021 (`STROMY_Web_Public`) s druhom a rokom → `stromy-mesta.json`. Plochy krajiny sme skúšali brať z vektorových dlaždíc ZBGIS na Geoportáli (Využitie krajiny, Voda — lesy podľa druhu, lúky, sady, vinice, potoky). Tie dlaždice však žiadajú prihlásenie: vracajú HTML stránku „Sign In“, takže vo verejnej mape sú nepoužiteľné. Zeleň, voda a budovy preto idú z podkladovej mapy OSM, ktorá je verejná.
...
**Návrhy polohy z mapy.** V karte zámeru je tlačidlo *Navrhnúť polohu na mape* (pri presných bodoch *Bod nesedí? Navrhnúť opravu polohy*). Používateľ klikne na mapu, bod môže potiahnuť, vyplní, kto návrh posiela, a odošle. Bez servera ide návrh ako verejné GitHub issue s názvom `Návrh polohy: …` a strojovým blokom `<!-- navrh-polohy {…} -->` (predvyplnené, stačí potvrdiť); kto GitHub nemá, skopíruje text a pošle ho mailom. Ak je v `komunita.json` nastavené `api`, návrh sa POST-ne tam. Redakcia pri štarte stiahne otvorené issues (bez tokenu, verejné API) a ukáže ich v paneli *Návrhy z mapy*: *Prevziať polohu* nastaví polohu do konceptu s poznámkou „podľa návrhu z mapy — kto“, *Uložiť na GitHub* ju zapíše; issue sa zavrie tokenom, ak má právo na Issues, inak ručne cez odkaz. Do mapy sa bez schválenia nič nezapíše.
